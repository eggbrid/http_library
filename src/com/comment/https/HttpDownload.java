...
/**
 * @author  王旭
 * @date 创建时间：2015年9月22日 下午3:02:31 
 * @version 1.0 
 * @parameter  
 * @since  
 * @return  
 */
public class HttpDownload implements Serializable {  
     ...
     */  
    public HttpDownload(int downloadId, String url, String localPath) {  
        if (!new File(localPath).exists()) {  
            new File(localPath).mkdirs();  
        }  
            
        Log.i("HttpDownload","下载地址；" + url);  
     ...
     * @return this 
     */  
    public HttpDownload setOnDownloadListener(OnDownloadListener listener) {  
        mListener = listener;  
        return this;  
    }  
...
//                  Log.i("HttpDownload",(int)(downloadedLength/(float)remoteFileLength * 100));  
                    if ((int)(downloadedLength/(float)remoteFileLength * 100) % 10 == 0) {  
                        ...
                        handler.sendMessage(msg);  
//                      Log.i("HttpDownload",mDownloadId + "已下载" + downloadedLength);  
                    }  
                    ...
                    if (isPause) {  
                        ...
                        Log.i("HttpDownload","下载暂停...");  
                        break;  
                    }  
                        
                    // 取消下载， 删除文件并退出方法  
                    if (isCanceled) {  
                        Log.i("HttpDownload","手动关闭下载。。");  
                        localFile.close();  
    ...
    public synchronized boolean pause(boolean pause) {  
        if(!pause) {  
            Log.i("HttpDownload","继续下载");  
            isPause = false;  
            start(true); // 开始下载  
        }else {  
            Log.i("HttpDownload","暂停下载");  
            isPause = true;  
        }  
        return isPause;  
    }  
    ...
    public synchronized long getLocalFileLength() {  
        ...
        Log.i("HttpDownload","本地文件大小" + size);  
    ...
    public synchronized long getRemoteFileLength() {  
        ...
        Log.i("HttpDownload","远程文件大小" + size);  
        return size;  
    }  
    
    /** 
     * 设置http参数 不能设置soTimeout 
     * @return GetParams http参数 
     */  
    ...
    public interface OnDownloadListener {  
        void onStart(int downloadId, long fileSize);  // 回调开始下载
        void onPublish(int downloadId, long size);    // 回调更新进度
        void onSuccess(int downloadId);               // 回调下载成功
        void onPause(int downloadId);                 // 回调暂停
        void onError(int downloadId);                 // 回调下载出错
        void onCancel(int downloadId);                // 回调取消下载
        void onGoon(int downloadId, long localSize);  // 回调继续下载
    }  
}  
